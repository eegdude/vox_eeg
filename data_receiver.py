...
import copy
from eeg_lstm import LSTM
import torch


mne.set_log_level(verbose="ERROR")

model = LSTM(40, 2, 10, 3)
model.load_state_dict(torch.load("/Users/tama-ts/Downloads/w2.pth", map_location=torch.device('cpu')))
...
class Processor(): #  transformer mixin?
        ...
        while 1:
            ...
            if self.nsamp != self.ns.eeg_nsamp: # new sample arrives
                ...
                if start_window > 0: # if there is at least conf.spectral_analyis_window samples
                    ...
                    start_lstm = self.nsamp - conf.lstm_window
                    if start_lstm>0:
                        self.window_lstm = self.ns.ring_buffer[:, start_lstm:self.nsamp]
                    else:
                        self.window_lstm = np.zeros((conf.lstm_window, 40))
    ...
    #         if self.nsamp != self.ns.eeg_nsamp: # new sample arrives
    #             self.nsamp = self.ns.eeg_nsamp
    #             start_window = self.nsamp - conf.lstm_window
    #             if start_window > 0: 
    #                 window = self.ns.ring_buffer[:, start_window:self.nsamp]
    #                 self.window_lstm = torch.Tensor(self.ring_buffer[-1]).view(1, 40, -1)
    #                 self.get_lstm()
                    # print(self.eeg.ring_buffer.shape, start_window, self.nsamp)
    ...
    def transform_eeg_to_osc_features(self):
        ...
        x = torch.Tensor(self.window_lstm).view(1,-1,40)
        out, h = model(x, h)
        out = out[0].detach().numpy()
        print(out)
        payload["valence"], payload["dominance"] = out[0], out[1]
    ...
    #     h = model.init_hidden(1)
    #     h = tuple([each.data for each in h])
    #     out, h = model(self.window_lstm, h)
    #     print(out[0])
    #     payload["valence"], payload["dominance"] = out[0], out[1]
